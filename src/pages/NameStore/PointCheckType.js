...
const { confirm } = Modal
const pageName = 'pointCheckType'

@Form.create()
@connect(({pointCheckType, loading}) => ({pointCheckType, loading: loading.models.pointCheckType}))
...
class CheckCar extends Component {
  ...
  columns = [{
    ...
    //     return (
    //       <Fragment>
    //         {/* {getButtonAuth(79) ? <a onClick={() => this.handleModalVisible('修改类别名称', record)}>修改</a>: null}
    //         {getButtonAuth(80) ?
  ...
  render() {
    const {pointCheckType:{data}, loading, form: {
        getFieldDecorator
      }} = this.props
    const {
      confirmLoading,
    ...
    return (
      <Fragment>
        ...
        {/* <Row>
            ...
            >
             {getButtonAuth(78) ?
              <Button onClick={() => this.handleModalVisible('新增类别')}
                  type="primary"
              >新建</Button>
              : null}
            </Col>
