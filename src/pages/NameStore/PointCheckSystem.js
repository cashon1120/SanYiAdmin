...
const { confirm } = Modal;
const pageName = 'pointCheckSystem'
@connect(({pointCheckSystem, loading}) => ({pointCheckSystem, loading: loading.models.pointCheckSystem}))
@Form.create()
...
class CheckCar extends Component {
  ...
  columns = [{
    ...
    name: 'name'
      ...
      render: (record) => {
        return (
          <Fragment>
            {getButtonAuth(76) ? <a onClick={() => this.handleModalVisible('修改系统名称', record)}>修改</a>: null}
            {getButtonAuth(75) ?
  ...
  render() {
    const {pointCheckSystem:{data}, loading, form: {
        getFieldDecorator
      }} = this.props
    const {
      confirmLoading,
    ...
    return (
      <Fragment>
        ...
        <Row>
            ...
            >
              {getButtonAuth(74) ?
              <Button onClick={() => this.handleModalVisible('新增系统')}
                  type="primary"
              >新建</Button>
              : null}
            </Col>
